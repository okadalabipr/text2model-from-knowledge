        ...
        for i, condition in enumerate(self.conditions):
            ...
            if sol is None:
                ...
                self.simulations[self.obs_names.index('IL13_cell'), i] = (
                    5.56750 * (sol.y[V.IL13_Rec] + sol.y[V.p_IL13_Rec] + sol.y[V.p_IL13_Reci] + sol.y[V.IL13_DecoyR])
                )
                self.simulations[self.obs_names.index('pIL4Ra'), i] = (
                    1.88700 * (sol.y[V.p_IL13_Rec] + sol.y[V.p_IL13_Reci])
                )
                self.simulations[self.obs_names.index('pJAK2'), i] = (
                    1.39040 * sol.y[V.pJAK2]
                )
                self.simulations[self.obs_names.index('SOCS3mRNA'), i] = (
                    17.66990 * sol.y[V.SOCS3mRNA]
                )
